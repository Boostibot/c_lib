...
#include "lpf2.h"
static void test_lpf2_entry_full(Lpf2_Entry* entry, Lpf2_Kind kind, const char* label, const char* value, i32 indentation, i32 blanks_before, i32 line)
{
    TEST(entry != NULL);
...
static void test_lpf2_entry(Lpf2_Entry* entry, Lpf2_Kind kind, const char* label, const char* value)
{
    test_lpf2_entry_full(entry, kind, label, value, -1, -1, -1);
}

static void test_lpf2_print_compariosn(String left, String right)
{
    isize max_left = 0;
    isize max_right = 0;
...
{
    {
        ...
        test_lpf2_entry_full(&root, LPF2_COLLECTION, "", "", 0, 0, 0);
        TEST(root.children_count == 4);
        TEST(root.children != NULL);

        test_lpf2_entry_full(&root.children[0], LPF2_ENTRY,     "first", "value ",                  1, 1, 2);
        test_lpf2_entry_full(&root.children[1], LPF2_ENTRY,     "second", "value\t\ncontinuation",    5, 1, 4);
        test_lpf2_entry_full(&root.children[2], LPF2_ENTRY,     "third*", "value escaped",          6, 2, 8);
        test_lpf2_entry_full(&root.children[3], LPF2_COMMENT,   "", "comment\n with continuation",  1, 0, 10);
...

    {
        ...
        test_lpf2_entry(&root.children[0], LPF2_ENTRY, "out", "value ");
        test_lpf2_entry(&root.children[1], LPF2_COLLECTION, "col1", "");
        
        Lpf2_Entry* col1 = &root.children[1];
        TEST(col1->children_count == 5);
        TEST(col1->children != NULL);
        Lpf2_Entry* col2 = &col1->children[3];
        Lpf2_Entry* col3 = &col1->children[4];

        test_lpf2_entry(&col1->children[0], LPF2_ENTRY, "inside1", "value1");
        test_lpf2_entry(&col1->children[1], LPF2_ENTRY, "inside2", "value2\ncontinuation");
        test_lpf2_entry(&col1->children[2], LPF2_COMMENT, "", "comment\n with continuation comment");
        test_lpf2_entry(&col1->children[3], LPF2_COLLECTION, "col2", "");
        test_lpf2_entry(&col1->children[4], LPF2_COLLECTION, "col3", "");
        
        TEST(col2->children_count == 1);
        TEST(col2->children != NULL);
        TEST(col3->children_count == 0);
        TEST(col3->children == NULL);
        test_lpf2_entry(&col2->children[0], LPF2_ENTRY, "key", "value");
...

    {
        ...
        String formatted = lpf2_write_from_root(&scratch_arena, root, &options);
        if(string_is_equal(formatted, expected) == false)
            test_lpf2_print_compariosn(formatted, expected);

        TEST(string_is_equal(formatted, expected));

        Lpf2_Entry root_roundtrip = lpf2_read(&scratch_arena, formatted, NULL);
        String formatted_roundtrip = lpf2_write_from_root(&scratch_arena, root_roundtrip, &options);
