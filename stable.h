...
EXTERNAL void  stable_test_invariants(const Stable* stable, bool slow_checks);
...
#include <string.h>
INTERNAL void _stable_check_invariants(const Stable* stable)
{
    (void) stable;
    #ifndef STABLE_DEBUG
    ...
    #if STABLE_DEBUG > 1
        stable_test_invariants(stable, true);
    #elif STABLE_DEBUG > 0
        stable_test_invariants(stable, false);
    #endif
}
...
EXTERNAL void stable_init_custom(Stable* stable, Allocator* alloc, isize item_size, isize item_align, uint32_t allocation_size)
{
    ...
    _stable_check_invariants(stable);
}
