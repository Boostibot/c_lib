...
#define SECOND_MICROSECONDS     1000000ll
#define SECOND_NANOSECONDS      1000000000ll

EXTERNAL int64_t epoch_time();       //returns time since the epoch in microseconds
EXTERNAL int64_t clock_ticks();      //returns precise as possible yet long term stable time since unspecified point in time (last boot) 
EXTERNAL int64_t clock_ticks_frequency(); //returns the frequency of clock_ticks
EXTERNAL int64_t clock_ns();         //returns time in nanoseconds since since unspecified point in time (last boot) 
...
#define MODULE_HAS_IMPL_TIME
    ...
    int64_t g_clock_sec_freq = 0;
        ...
        EXTERNAL int64_t clock_ticks_frequency()
        {
        ...
        EXTERNAL int64_t epoch_time()
        {
            ...
            __declspec(noinline) static int64_t _clock_ns_unusual()
            {
                ...
                uint64_t freq = (uint64_t) clock_ticks_frequency(); 
        ...
        EXTERNAL int64_t clock_ticks_frequency() { return (int64_t) 1000000000LL; }
        EXTERNAL int64_t clock_ns()         { return clock_ticks(); }
        EXTERNAL int64_t epoch_time()       
        { 
            struct timespec ts = {0};
    ...
    EXTERNAL double clock_sec_set(double to_time)
    {
        ...
        int64_t freq = clock_ticks_frequency();
